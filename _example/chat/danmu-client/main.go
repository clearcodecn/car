...
const text = `（观察者网讯）继上月28日二读通过后，香港立法会于4日继续审议《国歌条例草案》。据“东网”等港媒报道称，现场反对派议员提出了21项修正案，但全部都被否决。按照立法会主席梁君彦所订立的时间表，预计4日傍晚将进行草案的三读表决。此外，4日的立法会现场再度发生反对派议员撒泼闹事的情况，中午12时50分许，议员陈志全和朱凯迪突然冲向主席台，并向会议厅内投掷有臭味的不明液体，里面甚至还有白色的虫，这导致会议被迫暂停。朱凯迪事后称不明液体是“有机肥料”，大批警员和消防员先后抵达会议厅调查。
...
func randomStr(n int) string {
	var d []byte
	for i := 0; i < n; i++ {
		d = append(d, text[rand.Intn(len(text)-1)])
	}
	return string(d)
}
